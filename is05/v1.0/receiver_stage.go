...
type ReceiverStage struct {
	SenderID        null.String                `json:"sender_id,omitzero"` // ID of the Sender subscribed to by this Receiver. This will be null if the receiver has not been configured to receive anything, or if it is receiving from a non-NMOS sender.
	MasterEnable    bool                       `json:"master_enable"`      // Master on/off control for receiver
	Activation      Activation                 `json:"activation"`
	TransportFile   ReceiverStageTransportFile `json:"transport_file,omitzero"` // Transport file parameters. 'data' and 'type' must both be strings or both be null
...
type ReceiverStageTransportFile struct {
