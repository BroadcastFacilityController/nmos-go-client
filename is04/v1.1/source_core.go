...
type SourceCore struct {
	ResourceCore
	GrainRate SourceCoreGrainRate `json:"grain_rate,omitzero"` // Maximum number of Grains per second for Flows derived from this Source. Corresponding Flow Grain rates may override this attribute. Grain rate matches the frame rate for video (see NMOS Content Model). Specified for periodic Sources only.
