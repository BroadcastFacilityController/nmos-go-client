...
type FlowCore struct {
	ResourceCore
	GrainRate FlowCoreGrainRate `json:"grain_rate,omitzero"` // Number of Grains per second for this Flow. Must be an integer division of, or equal to the Grain rate specified by the parent Source. Grain rate matches the frame rate for video (see NMOS Content Model). Specified for periodic Flows only.
